...
class Nutrient(BaseModel):
    ...
    name: str = Field(..., description="Extract the name of the nutrient (e.g., 'Total Fat', 'Sodium', 'Sugars').")
    ...
    unit: Optional[str] = Field(None, description="Extract the unit of measurement for the nutrient (e.g., 'g', 'mg', 'kcal').")
...
class Ingredient(BaseModel):
    ...
    name: str = Field(..., description="Extract the name of a single ingredient from the ingredients list (e.g., 'Enriched Flour', 'Palm Oil').")
    quantity: Optional[str] = Field(None, description="If a quantity or percentage is listed next to an ingredient, extract it exactly as a string (e.g., '5%', 'contains 2% or less of').")
...
class Dimensions(BaseModel):
    ...
    unit: Optional[str] = Field("cm", description="Extract the unit of measurement for the dimensions (e.g., 'cm', 'in', 'mm').")
...
class ProductContent(BaseModel):
    ...
    unit: str = Field(..., description="Extract the unit of the product's net weight or volume (e.g., 'g', 'kg', 'ml', 'L', 'oz').")
...
class ProductInfo(BaseModel):
    ...
    product_name: str = Field(..., description="Extract the most prominent, primary name of the product from the front of the package.")
    brand: Optional[str] = Field(None, description="Identify and extract the brand name, which is often a logo or located near the product name.")
    flavor: Optional[str] = Field(None, description="Extract the specific flavor of the product if it is mentioned (e.g., 'Chocolate Chip', 'Lemon Lime').")
    product_description: Optional[str] = Field(None, description="Extract any general descriptive marketing text or slogan from the package, if visible.")
    ...
    serving_size: Optional[str] = Field(None, description="From the nutrition facts panel, extract the serving size text (e.g., '1 cup (228g)').")
    servings_per_container: Optional[str] = Field(None, description="From the nutrition facts panel, extract the number of servings per container (e.g., 'about 2.5', '8').")
    ...
    allergens: Optional[List[str]] = Field(None, description="Extract the exact text from any dedicated allergen statement, often starting with 'Contains:' (e.g., 'Contains Wheat, Milk, and Soy.').")
    claims: Optional[List[str]] = Field(None, description="Extract any marketing or health claims made on the packaging (e.g., 'High in Fiber', 'No Added Sugar', 'Gluten Free').")
    certifications: Optional[List[str]] = Field(None, description="Extract the names of any official certifications shown as logos or text (e.g., 'USDA Organic', 'Halal', 'Non-GMO Project Verified').")
    
    manufacturer: Optional[str] = Field(None, description="Extract the company name listed after a phrase like 'Manufactured by:' or 'Produced by:'.")
    distributor: Optional[str] = Field(None, description="Extract the company name listed after a phrase like 'Distributed by:', if present and different from the manufacturer.")
    country_of_origin: Optional[str] = Field(None, description="Extract the country name from phrases like 'Made in', 'Product of', or 'Originated from'.")
    
    storage_instructions: Optional[str] = Field(None, description="Extract any instructions for storing the product (e.g., 'Refrigerate after opening', 'Store in a cool, dry place').")
